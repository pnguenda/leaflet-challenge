...
// I received help from Erin (TA) along with Justin Forkert, Corrine Ptacek, Naomi Scully, and Nicole Langford
...
function cicleSize(feature){
  var magSize = +feature.properties.mag  // 1.29

  radius = magSize**3   // magsize^3 
...
function colorCode(feature) {   // depth
  var depthValue = +feature.geometry.coordinates[2]
...
d3.json(queryUrl, function(data) {
  ...
  L.geoJSON(data, {
    style: function(feature) {
      return {
        color: colorCode(feature)    // "green"  // color(depth)
      };
  },
    onEachFeature: popUpMsg,
    pointToLayer: function(feature, latlng) {
      return new L.CircleMarker(latlng, {
        radius: cicleSize(feature),  //5   // radius is function (magnitude) 
        fillOpacity: 1
      });
  },
  }).addTo(earthquakes);
...
legend.onAdd = function (map) {
    ...
    var div = L.DomUtil.create('div', 'info legend'),   // create a <div class = "info legend"> <div>loop1, loop2, loop3
        grades = [0, 2, 10],
        labels = ["green", "yellow", "red"];  //what are labels
        
        div.innerHTML += "<h3> Earthquake Depth (mi)<h3>"
       // div.innerHTML += "" //create based on a list
